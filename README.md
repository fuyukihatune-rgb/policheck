...
**🔗 ライブデモ: https://policheck.onrender.com/** ／ CLI・MCP・Web の3エントリで動作
（Render Freeのため初回アクセスはコールドスタート数十秒）
...
**連携先に Apps in Claude（MCP Apps）を選定した理由**：本ツールの中核は「Tool Useループを自前実装したエージェント」であり、MCP（Model Context Protocol）の標準サーバーとして公開すれば Claude Desktop 等からそのまま呼べる。MCP は OpenAI Apps SDK と異なりベンダー非依存のオープン仕様で、CLI／Web と**同一のエージェント実装を共有**できる点も決め手。UI（`ui://`）は今回スコープ外とし、ツール公開（`check_policy` 等）に絞った。
